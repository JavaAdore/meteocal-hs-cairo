...
@ManagedBean
@ViewScoped
public class EditEventHandlerBean implements Serializable {
	...
	public void init() { 
		...
		Event event = (Event) WebUtils
				.extractFromSession(Constants.SELECTED_EVENT);
		if (event == null) {
		...
		if (Util.isNotEmpty(badWeathers)) {
			if (event.getBadWeather() != null) {
				badWeather = event.getBadWeather();
			} else {
		...
		lastAvailableTimeToAcceptOrInvite = event.getLastAvailableTimeToAcceptOrInvite(); 
		description = event.getDescription();
		try {
			latlng = new LatLng(Double.valueOf(event.getLat()),
					Double.valueOf(event.getLon()));
		} catch (Exception e) { 
		}
	...
	public void addEvent(boolean processAnyWay) {
		...
		try {
			eventFacade.editEvent( id,eventTitle, currentMember, eventType,
			...
			WebUtils.fireInfoMessage(Constants.ADD_EVENT_EVENT_EDITED_SUCCESSFULLY);
	...
	public void selectionChange() {
		if (badWeather != null) {
			...
			if (badWeather != null) {
				WebUtils.fireExactInfoMessage(
						badWeather.getName(),
						String.format(" ( min:%s , max : %s)",
								badWeather.getMinDegree(),
								badWeather.getMaxDegree()));
			}
		}
	}
